...
2 Hypothèses, après avoir coupé la valeur dehors 0 et 100, reprocéder
```{r}
diff_in_mean <- m_good - m_bad
...
En conséquence, Dans les grandes données et les 3 observations des données, il existe la risque pour adopter l'hypothèse 1 et l'hypothèse erronées. Dans le cas de 1ère hypothèse, il existe la risque pour ne conclure les erreures sur la différence entre deux groupes. Dans le cas de 2ème hypothèse, il y a la risque car il prend beacoup de différences de la moyenne d'entre grandes données et 3 observations données (petites données) : 60 vs 40.
Et, la différence de la question 2 et 3, est la taille des données. on peut savoir que plus les tailles des données sera petites, plus la force statistique réduit car deux graphiques se chevauchent